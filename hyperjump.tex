...
\end{equation}
with five-dimensional Minkowski signature $\eta_{MN} = \mathrm{diag}(-1, +1, +1, +1, +1)$ in natural units ($\hbar=c=1$) and implicit sums over $M,N$ and $A$. Structurally this is the Higgs sector with the field-space vacuum manifold $\{\phi^A\phi^A=v^2\}\cong S^3$. The physical spatial hypersphere remains the geometric locus $\lvert X\rvert=\Rgeom$; identifying that locus with the low-energy brane requires a background profile $\phi_0^A(r)$, where $r=\lvert X\rvert-\Rgeom$, that localizes Standard-Model zero modes near $r=0$. We emphasize that such a radial domain-wall profile at a specific finite radius $|X|=\Rgeom$ does \emph{not} form spontaneously in flat $\mathbb{R}^4$ from the potential $\lambda(|\phi|^2-v^2)^2$ alone --- the natural codimension-four defect of an $O(4)$-symmetric model is a global-monopole hedgehog $\phi^A\propto \hat X^A$ centred at $|X|=0$, not a thin shell at finite radius. Pinning the brane at $|X|=\Rgeom(\tau)$ requires the additional dynamical input of Subproblems~1 and 2 (cosmic-expansion sector and coupling to gravity); $\phi_0^A(r)$ should therefore be read as the ansatz one wishes a self-consistent solution to produce, not as an established solution of the present action.
...
Excitations with energy density above the central barrier $\rho_\star=\lambda v^4$ can in principle push localized states into the bulk along chord trajectories; excitations below the barrier tunnel quantum-mechanically. The semiclassical estimate must, however, be adapted to the symmetry of the potential: $V(\phi)=\lambda(\phi^A\phi^A-v^2)^2$ has two \emph{degenerate} minima on the vacuum manifold $|\phi|=v$, so the standard Coleman false-vacuum bounce [Coleman 1977] does not apply --- there is no finite-action $O(d)$-symmetric instanton that starts at $|\phi|=v$, passes through $|\phi|=0$, and returns. The correct saddle is instead the planar domain wall (kink) of surface tension
\begin{equation}
\sigma \;=\; \tfrac{4\sqrt{2}}{3}\,\sqrt{\lambda}\,v^3,
\label{eq:wall-tension}
\end{equation}
and the Euclidean action for a cargo of transverse 3-area $A_{\rm c}$ traversing a chord of length $\ell_{\mathrm{chord}}=2\Rgeom\sin(\theta/2)$ is dominated by the worldvolume of the wall-tube wrapping that cargo,
\begin{equation}
B_{\rm phys} \;\sim\; \sigma\,\bigl(P_{\rm c}\,\ell_{\mathrm{chord}} + 2\,A_{\rm c}\bigr),
\label{eq:tube-action}
\end{equation}
with $P_{\rm c}$ the 2-perimeter of the cargo transverse to the chord. Once gravity is included, the appropriate extension of~\eqref{eq:tube-action} is a Coleman--De Luccia-type calculation [Coleman \& De Luccia 1980] for the wall worldvolume on the curved bulk, not for a Coleman bounce. The parameters $\lambda$ and $v$ then control the \emph{brane thickness} $\ell_\perp \sim 1/(\sqrt{\lambda}\,v)$, the \emph{barrier energy density} $\rho_\star=\lambda v^4$, the \emph{wall tension}~\eqref{eq:wall-tension}, and via~\eqref{eq:tube-action} the parametric \emph{action cost} of transporting an object through the bulk.
...
\item \textbf{Cosmic expansion.} [Nopper 2025a] has $\Rgeom=\Rgeom(\tau)$ (FLRW). The static brane location in the skeleton is incompatible. \emph{First-pass closure:} introduce an additional collective coordinate $\Phi(\tau)$ controlling the equilibrium brane radius and a potential $U(\Phi)$ with a kinetic term $-\tfrac{1}{2}(\partial\Phi)^2$. (We emphasise: $\Phi$ is an effective field controlling the brane location in the embedding $\mathbb{R}^4$; it is \emph{not} the FLRW scale factor itself, which must instead emerge from the induced 4D metric of Subproblem 2.) The homogeneous mode then satisfies $\ddot\Phi+3H\dot\Phi+U'(\Phi)=0$, the standard scalar-field-cosmology equation [Linde 1990]; for slow-roll choices of $U$ this reproduces de Sitter expansion. Full closure requires coupling to 5D gravity (Subproblem 2) so that $H$, the relation between $\Phi$ and the sigma-model profile, and the resulting FLRW scale factor are all solutions rather than postulates.
...
\end{equation}
in the spirit of Randall--Sundrum-type braneworlds. \emph{First-pass closure:} in the thin-brane limit $\ell_\perp \to 0$, the energy-momentum tensor of the localized $\phi_0^A(r)$ background degenerates to a delta-function shell source near $\lvert X\rvert = \Rgeom$ and the 5D Einstein equations reduce to Israel junction conditions [Israel 1966]; the induced 4D geometry is then standard FLRW with the brane tension $\sigma = \int dr\, V(\phi_0)$ playing the role of an effective cosmological constant. A treatment with finite $\ell_\perp$ requires solving the coupled $(g_{MN}, \phi^A)$ system numerically; with the warped ansatz $ds^2=e^{2A(r)}\eta_{\mu\nu}dx^\mu dx^\nu+dr^2$ the system admits an explicit DeWolfe--Freedman--Gubser--Karch first-order flow $\phi'(r)=W'(\phi)$, $A'(r)=-W(\phi)/3$ with induced potential $V(\phi)=\tfrac12 W'(\phi)^2-\tfrac23 W(\phi)^2$ [DeWolfe et al.\ 2000]. Choosing $W(\phi)=\phi-\phi^3/(3v^2)$ gives the closed-form domain-wall background $\phi_*(r)=v\tanh(r/v)$, $A_*(r)=-\tfrac{2}{9}\ln\cosh(r/v)-\tfrac{1}{18}\tanh^2(r/v)$, which we reproduce numerically with a physics-informed neural network in the companion code (\texttt{numerics/pinn\_coupled.py}). This is an explicit self-consistent 5D background of the type the hyperjump skeleton requires.
...
\item \textbf{Determination of $\lambda$ and $v$.} The coupling and vev are unconstrained, so the model is not predictive. \emph{First-pass closure:} in the present setup the transverse direction (radial in the embedding $\mathbb{R}^4$, across the $S^3$ brane) is \emph{non-compact}, so $\ell_\perp\sim1/(\sqrt{\lambda}\,v)$ is a brane-thickness / warping length, not a Kaluza--Klein compactification radius. Current laboratory bounds on deviations from the Newtonian inverse-square law constrain a new transverse length scale to $\ell_\perp \lesssim 52\,\mu\mathrm{m}$ at 95\,\% C.L.\ [Lee et al.\ 2020], superseding the earlier $\sim 44\,\mu\mathrm{m}$ bound reviewed in [Adelberger et al.\ 2003]; this translates into $\sqrt{\lambda}\,v\gtrsim 1/\ell_{\exp}\sim 4\,\mathrm{meV}$ at the present level of precision. This constrains only the product $\sqrt{\lambda}\,v$; pinning unique values requires an independent observable (e.g.\ a continuum graviton signature in a non-compact transverse direction, a tunnelling rate, a radial-mode mass, or a cosmological constraint on $\Phi$). The precise translation of the laboratory bound into a constraint on $\sqrt{\lambda}\,v$ depends on the unsolved gravity sector (Subproblem 2) and is therefore itself only first-pass.

\item \textbf{Operational FTL versus [Nopper 2025a]'s prohibition.} Chord traversal is timelike in the bulk but superluminal in $S^3$ projection. \emph{First-pass closure:} adopt the bulk Minkowski light-cone of $\eta_{MN}=\mathrm{diag}(-1,+1,+1,+1,+1)$ (natural units) as the \emph{fundamental} causal structure, and reinterpret [Nopper 2025a]'s intrinsic light-speed bound as an emergent statement about $S^3$-confined matter --- strictly valid only for trajectories that remain on the brane. Chord trajectories then satisfy the bulk light-cone. The standard ``FTL implies CTCs by Lorentz-boosting'' argument does \emph{not} directly apply here, because the FLRW embedding picks out a preferred cosmic-time slicing and brane Lorentz boosts are not a true symmetry of the full theory. \emph{However}, this observation alone does not constitute a chronology-protection proof: brane-confined observers in different regions of $S^3$ enjoy \emph{approximate} local Lorentz invariance, and one must verify explicitly that no concatenation of (i) brane-confined sub-light trajectories and (ii) allowed bulk chords can close into a causal loop with respect to the bulk light-cone. We do not supply that verification here; it is an explicit open subproblem of this note. The cost is conceptual: the ``intrinsic'' formulation of light-speed in [Nopper 2025a] must be downgraded to an effective brane-projected bound, and Lorentz invariance of the brane physics becomes approximate rather than exact (constrained by precision tests of vacuum Lorentz invariance --- see Tier 1 below). An alternative --- adding a chronology-protection term that suppresses access to $\phi^A\approx0$ --- preserves the intrinsic formulation but reintroduces an arbitrary suppression scale.

\item \textbf{Tunnelling rate.} A field-theoretic semiclassical calculation should replace the 1D-WKB heuristic. \emph{First-pass closure:} the symmetric double-well $V(\phi)=\lambda(\phi^A\phi^A-v^2)^2$ has two \emph{degenerate} vacua on $|\phi|=v$, so a Coleman-type $O(d)$-symmetric bounce [Coleman 1977] interpolating between $|\phi|=v$ at infinity and $|\phi|=0$ at the centre does not exist as a finite-action saddle --- the relevant process is not false-vacuum decay. The appropriate saddle is the planar kink with surface tension $\sigma=\tfrac{4\sqrt 2}{3}\sqrt\lambda\,v^3$ (Eq.~\ref{eq:wall-tension}); the Euclidean action for a cargo traversing a chord of length $\ell_{\mathrm{chord}}$ is dominated by the wall-tube worldvolume,
\begin{equation*}
B_{\rm phys} \;\sim\; \sigma\bigl(P_{\rm c}\,\ell_{\mathrm{chord}} + 2A_{\rm c}\bigr),
\end{equation*}
giving $\Gamma/\mathcal{V}_5\sim\mu^5 e^{-B_{\rm phys}/\hbar}$. Substituting the laboratory-saturating value $\sqrt{\lambda}\,v\sim 1/\ell_\perp\sim 4\,\mathrm{meV}$ from Subproblem~5 and any macroscopic cargo (e.g.\ a $1\,\mathrm{m}$-radius object across a $1\,\mathrm{ly}$ chord) gives $B_{\rm phys}\gtrsim10^{30}$, so unaided hyperjumps are exponentially suppressed beyond microscopic probes and would require an external coherent source to occur on macroscopic timescales. Once gravity is included, the appropriate extension is a Coleman--De Luccia-type computation [Coleman \& De Luccia 1980] of the wall worldvolume on the curved bulk; this fixes the exponent's $O(1)$ coefficient and the prefactor. A companion numerical implementation (\texttt{numerics/bounce.py} in this repository) verifies $\sigma$ analytically and numerically and tabulates $B_{\rm phys}$ for sample cargos.
...
\midrule
Sub-mm gravity (E\"ot-Wash and successors) & Brane thickness $\ell_\perp$ constrains $\sqrt{\lambda}\,v$ via Subproblem 5 & Currently $\ell_\perp\lesssim 52\,\mu\mathrm{m}$ (95\,\% C.L.) [Lee et al.\ 2020], superseding $\sim 44\,\mu\mathrm{m}$ [Adelberger et al.\ 2003]; next-generation aims at the micron scale \\
...
\item {[DeWolfe et al.\ 2000]} DeWolfe, O., Freedman, D.~Z., Gubser, S.~S.\ \& Karch, A. ``Modeling the fifth dimension with scalars and gravity.'' \emph{Physical Review D} 62, 046008 (2000).
\end{itemize}
...
\item {[Lee et al.\ 2020]} Lee, J.~G., Adelberger, E.~G., Cook, T.~S., Fleischer, S.~M.\ \& Heckel, B.~R. ``New Test of the Gravitational $1/r^2$ Law at Separations down to $52\,\mu$m.'' \emph{Physical Review Letters} 124, 101101 (2020).
\end{itemize}
