...
2. **Coherent-source proof of principle.** A configuration raising $q^A$ above the central barrier in a small region. Saturating Subproblem 5's lower bound $\sqrt\lambda \geq 1/(\ell_\exp R)$ gives a required energy density $\rho_\star = \lambda R^4 \geq R^2/\ell_\exp^2$ in natural units, which for $R$ of order the Hubble radius and $\ell_\exp \sim 44\,\mu\text{m}$ corresponds to densities astronomically larger than anything currently producible — comparable to the negative-energy-density requirements of warp/wormhole proposals, though here with positive sign.
